...
import {Configuration, DieConfiguration, THEME_CONFIGS, ThemeEnum} from "../../src/app/general_types";
import {OperationEnum, OPERATIONS_MAP} from "../../src/app/solver/solver";

export const MIN_SIZE_QUERY = 'app-configuration [formgroupname="board"] [formcontrolname="minSize"]';
export const MAX_SIZE_QUERY = 'app-configuration [formgroupname="board"] [formcontrolname="maxSize"]';
export const FACE_COUNT_QUERY = 'app-configuration [formArrayName="dice"] input[formcontrolname="faceCount"]';
const selectedFaceQuery = 'app-configuration [formArrayName="dice"] input[formcontrolname="selectedFace"]';

export const sortOperations = (operations: OperationEnum[]): OperationEnum[] => {
  const sortedOperations = [...operations];
  sortedOperations.sort();
  return sortedOperations;
}

export const standardizeConfiguration = (configuration: Configuration): Configuration => {
  return {
...
export const clickToolbarCloseButton = async (page: Page) => {
  await page.locator('app-toolbar button[data-test-id="app-configuration_toolbar_closeButton"]').click();
  await page.waitForSelector('app-configuration', {
    state: "detached",
  });
...
export const clickResetToDefaultsButton = async (page: Page) => {
  await page.locator('app-configuration [data-test-id="app-configuration_resetToDefaults"]').click();
};

export const getTheme = async (page: Page): Promise<ThemeEnum> => {
  const selected = await page.locator('app-configuration [formcontrolname="theme"] .mat-list-single-selected-option');
  const value = await selected.getAttribute("data-value");

  const matchingTheme = THEME_CONFIGS
...
export const setTheme = async (page: Page, theme: ThemeEnum) => {
  await page.locator(`app-configuration [formcontrolname="theme"] [data-value="${theme}"]`).click();
  await waitForAnimationToSettle();
  expect(await getTheme(page)).toEqual(theme);
};

export const getOperations = async (page: Page): Promise<OperationEnum[]> => {
  const operations = sortOperations(await mapLocator(
    page.locator('app-configuration [formcontrolname="operations"] [aria-selected="true"]'),
    (itemLocator) => itemLocator.getAttribute("data-value")
  ));

  operations.forEach((operation) => expect(OPERATIONS_MAP[operation]).toBeTruthy());
  return operations;
};


export const setOperations = async (page: Page, operations: OperationEnum[]) => {
  await forEachLocator(
    page.locator('app-configuration [formcontrolname="operations"] mat-list-option'),
    async (itemLocator) => {
      const isSelected = await itemLocator.getAttribute("aria-selected") === "true";
      ...
      const shouldClick = (!isSelected && shouldBeSelected) || (isSelected && !shouldBeSelected);
      if (shouldClick) {
...
export const setDieFaceCount = async (page: Page, dieIndex: number, faceCount: number) => {
  const initial = await getDice(page);
  await page.locator(FACE_COUNT_QUERY).nth(dieIndex).fill(faceCount + "");
...
export const clickAddDieButton = async (page: Page) => {
  ...
  await page.locator('app-configuration [formArrayName="dice"] button.addDie').click();
  await page.waitForFunction(
    (diceCount) => diceCount === document.querySelectorAll('[formArrayName="dice"] input[formcontrolname="faceCount"]').length,
    diceConfigInitial.length + 1
  );
  expect(await getDice(page)).toEqual(diceConfigInitial.concat({
    faceCount: 6,
    selectedFace: 1,
  }));
};
...
export const clickRemoveDieButton = async (page: Page) => {
  const diceConfigInitial = await getDice(page);
  await page.locator('app-configuration [formArrayName="dice"] button.removeDie').click();
  await page.waitForFunction(
    (diceCount) => diceCount === document.querySelectorAll('[formArrayName="dice"] input[formcontrolname="faceCount"]').length,
    diceConfigInitial.length - 1
  );
...
export const setConfiguration = async (page: Page, configuration: Configuration) => {
